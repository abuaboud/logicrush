...
| **Golden replay (scoring parity)** | ✅ | `parity:replay` recomputes every past contest from migrated submissions: **25/26 contests reproduce the recorded ranks exactly** (1098/1230 contestants); the one outlier (iiylo) diverges because legacy rated 231 of 270 registrants and the rebuilt scoreboard ranks a scored participant legacy excluded from the rated set, shifting ranks by one below that row — a scoreboard-vs-legacy-rating-population difference, not a scoring-formula defect (points and the migrated rating history reproduce faithfully). |
